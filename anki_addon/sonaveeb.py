...
# Dictionary types and their URLs
DICTIONARY_TYPES = {
    'LITE': {
        'name': 'lite',
...
DEFAULT_DICTIONARY = 'UNIF'

class Sonaveeb:
    def __init__(self, dict_type=DEFAULT_DICTIONARY):
    ...
    def _word_lookup_dom(self, word):
        ...
        url = DICTIONARY_TYPES[self.dict_type]['urls']['search'].format(word=word)
        resp = self._request(url)
        return bs4.BeautifulSoup(resp.text, 'html.parser')

    def _word_details_dom(self, word_id):
        self._ensure_session()
        url = DICTIONARY_TYPES[self.dict_type]['urls']['details'].format(word_id=word_id)
        resp = self._request(url)
        return bs4.BeautifulSoup(resp.text, 'html.parser')
    ...
    def _parse_word_info(self, dom):
        ...
        for match in dom_to_parse.find_all(id=re.compile('^lexeme-section')):
            ...
            lexeme.definition = definition
            lexeme.level = level
            lexeme.translations = translations
            lexeme.examples = examples
            lexeme.tags = tags
            lexeme.synonyms = synonyms
    ...
    def get_forms(self, word):
        self._ensure_session()
        url = DICTIONARY_TYPES[self.dict_type]['urls']['forms'].format(word=word)
        resp = self._request(url)
        data = resp.json()
