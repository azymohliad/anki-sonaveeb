...
class SonaveebDialog(QWidget):
    def __init__(self, sonaveeb=None, parent=None):
        ...
        self._deck_selector.setMaximumWidth(300)
        ...
        self._dict_selector.addItem('Lite', userData='LITE')
        self._dict_selector.addItem('Unif', userData='UNIF')
        ...
        index_l = self._lang_selector.findData(lang)
        if index_l >= 0:
            self._lang_selector.setCurrentIndex(index_l)

        # Initialize dictionary type from config
        default_dict_type = 'LITE'
        dict_type = self._config.get('dictionary', default_dict_type)
        index_d = self._dict_selector.findData(dict_type)
        if index_d >= 0:
            self._dict_selector.setCurrentIndex(index_d)
    ...
    def dictionary_changed(self, _index):
        ...
        self._sonaveeb.dict_type = dict_type
